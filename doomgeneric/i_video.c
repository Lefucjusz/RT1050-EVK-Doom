...
//
// I_FinishUpdate modified for RT1050-EVK display
// - converts Doom's video buffer to RGB565
// - naively scales image from Doom resolution to display resolution
//

#define SCALE_TO_SCREEN_X(x) (SCREENWIDTH * (x) / DISPLAY_WIDTH)
#define SCALE_TO_SCREEN_Y(y) (SCREENHEIGHT * (y) / DISPLAY_HEIGHT)

void I_FinishUpdate(void)
{
	for (size_t y = 0; y < DISPLAY_HEIGHT; ++y) {
		const size_t y_src = SCALE_TO_SCREEN_Y(y);
		const uint8_t *line = &I_VideoBuffer[y_src * SCREENWIDTH];
		for (size_t x = 0; x < DISPLAY_WIDTH; ++x) {
			const size_t x_src = SCALE_TO_SCREEN_X(x);
			const struct color c = colors[line[x_src]];

			/* Convert to RGB565 */
			const uint16_t r = ((uint16_t)(c.r >> 3)) << 11;
			const uint16_t g = ((uint16_t)(c.g >> 2)) << 5;
			const uint16_t b = ((uint16_t)(c.b >> 3)) << 0;
			DG_ScreenBuffer[y * DISPLAY_WIDTH + x] = (r | g | b);
		}
	}
}

//
// I_FinishUpdate ORIGINAL
//
